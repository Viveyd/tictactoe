const Game = (function () {
  ...
  function checkAiTurn() {
    if (
      gameBoard.player.player2.type == "AI" &&
      matchState.currentMarker == gameBoard.player.player2.mark
    )
      AImoves();
  }
  ...
  function mapBoard() {
    ...
    let columns = [
      [...gameBoard.con.querySelectorAll(".column-1")],
      [...gameBoard.con.querySelectorAll(".column-2")],
      [...gameBoard.con.querySelectorAll(".column-3")],
    ];
    for (let i = 1; i <= columns.length; i++) {
      gameBoard.column[i] = columns[i - 1];
    }
    let diagonals = [
      [...gameBoard.con.querySelectorAll(".diagonal-1")],
      [...gameBoard.con.querySelectorAll(".diagonal-2")],
    ];
  ...
  function markCell(e, aiMove) {
    ...
    if (getMark(cell) == "none") {
      ...
      matchState.unmarkedCells.splice(
        matchState.unmarkedCells.indexOf(cell),
        1
      );
      changeMarker();
  ...
  function changeMarker() {
    matchState.currentMarker =
      matchState.currentMarker == matchState.marker[0]
        ? matchState.marker[1]
        : matchState.marker[0];
  }
  // AI move processing
  ...
  // Strat-1: Random
  function randomMove() {
    return matchState.unmarkedCells[
      Math.floor(Math.random() * matchState.unmarkedCells.length)
    ];
  }
  // Strat-2: Minimax
  ...
  function computeCell(cell) {
    if (matchState.markerCount == 0) {
      if (
        [cells[0], cells[2], cells[5], cells[6], cells[8]].indexOf(cell) != -1
      )
        return 3;
      else return 1;
  ...
  function isAiFinisher(cell, mark) {
    return isRowFinisher(cell, mark) ||
      isColumnFinisher(cell, mark) ||
      isDiagonalFinisher(cell, mark)
      ? true
      : false;
  }
  ...
  function isRowFinisher(cell, mark) {
    ...
    let filtered = [...row.children].filter(
      (lineCell) => lineCell != cell && getMark(lineCell) == mark
    );
    return filtered.length == 2 ? true : false;
  }
  function isColumnFinisher(cell, mark) {
    let column = getColumnOf(cell);
    let filtered = column.filter(
      (lineCell) => lineCell != cell && getMark(lineCell) == mark
    );
    return filtered.length == 2 ? true : false;
  }
  function isDiagonalFinisher(cell, mark) {
    if (
      cell.classList.contains("diagonal-1") ||
      cell.classList.contains("diagonal-2")
    ) {
      let diagonal = getDiagonalOf(cell);
      if (diagonal.length != 2) {
        let filtered = diagonal.filter(
          (lineCell) => lineCell != cell && getMark(lineCell) == mark
        );
        return filtered.length == 2 ? true : false;
      } else {
        let filteredA = diagonal[0].filter(
          (lineCell) => lineCell != cell && getMark(lineCell) == mark
        );
        let filteredB = diagonal[1].filter(
          (lineCell) => lineCell != cell && getMark(lineCell) == mark
        );
  ...
  function computeCell2(cell) {
    ...
    if (matchState.markerCount == 0) {
      ...
      matchState.markerCount == 1 &&
      cell == cells[4] &&
      humanDiagonalStart(firstCellMarked)
    )
      return 4;
    else {
      let aiMark = gameBoard.player.player2.mark;
      ...
      else if (matchState.markerCount > 2 && isHumanFinisher(cell, humanMark))
        return 5;
      else if (
        matchState.markerCount == 3 &&
        [cells[0], cells[2], cells[4], cells[6], cells[8]].indexOf(cell) ==
          -1 &&
        humanDiagonalStart(firstCellMarked)
      )
        return generateReturnVal(cell, firstCellMarked);
  ...
  function humanDiagonalStart(firstCell) {
    if (
      getMark(firstCell) == gameBoard.player.player1.mark &&
      [cells[0], cells[2], cells[6], cells[8]].indexOf(firstCell) != -1
    ) {
      return true;
    } else return false;
  }
  ...
  function isValidFork(cell, humanMark, firstCell) {
    if ([cells[0], cells[2], cells[6], cells[8]].indexOf(cell) != -1) {
      if (
        matchState.markerCount == 2 &&
        getMark(firstCell) == gameBoard.player.player2.mark
      ) {
        ...
        if (
          getMark(gameBoard.cell[3]) != humanMark &&
          getMark(gameBoard.cell[7]) != humanMark
        )
          return true;
        else if (
          cell == gameBoard.cell[1] &&
          getMark(gameBoard.cell[9]) != humanMark
        )
          return true;
        else if (
          cell == gameBoard.cell[9] &&
          getMark(gameBoard.cell[1]) != humanMark
        )
          return true;
        else return false;
        ...
        if (
          ...
          cell == gameBoard.cell[3] &&
          getMark(gameBoard.cell[7]) != humanMark
        )
          return true;
        else if (
          cell == gameBoard.cell[7] &&
          getMark(gameBoard.cell[3]) != humanMark
        )
          return true;
        else return false;
  ...
  function initNextMatch(loserMark) {
    ...
    if (gameBoard.currentMatch > gameBoard.matchLimit) gameOver();
      ...
      gameBoard.player.player2.type == "AI" &&
      loserMark == gameBoard.player.player2.mark
    )
      AImoves();
  }
  ...
  function gameOver() {
    let winner =
      gameBoard.player.player1.score > gameBoard.player.player2.score
    ...
    if (gameBoard.player.player1.score != gameBoard.player.player2.score) {
      if (gameBoard.player.player2.type == "AI") {
        if (winner == gameBoard.player.player2)
          alert(
            `At ${winner.score}-${loser.score}, you were dominated by Mimir's superior intellect.`
          );
        else
          alert(
            `${winner.score}-${loser.score}. You won. Mimir found his successor.`
          );
      } else {
        alert(
          `${winner.name} won with a score of ${winner.score}-${loser.score} against ${loser.name}!`
        );
      }
    } else {
      if (gameBoard.player.player2.type == "AI")
        alert(
          `You went toe-to-toe with Mimir and held your ground. Mimir is impressed.`
        );
      else
        alert(
          `${winner.name} tied with ${winner.name} at ${winner.score}-${loser.score} in an equal display of skill.`
        );
    }
    quitMatch();
  }
  ...
  function getOtherPlayer(player) {
    return player == gameBoard.player.player1
      ? gameBoard.player.player2
      : gameBoard.player.player1;
  }

  function getPlayerByMark(mark) {
    return gameBoard.player.player1.mark == mark
      ? gameBoard.player.player1
      : gameBoard.player.player2;
  }
  ...
  function evalAdjacents(cell) {
    return evalRow(cell) || evalColumn(cell) || evalDiagonal(cell)
      ? true
      : false;
  }
  ...
  function evalDiagonal(cell) {
    if (
      ...
      if (diagonal.length != 2) {
        ...
        return isLineComplete(filteredA) || isLineComplete(filteredB)
          ? true
          : false;
      }
    } else return false;
  ...
  function getDiagonalOf(cell) {
    return cell == gameBoard.cell[5]
      ? [gameBoard.diagonal[1], gameBoard.diagonal[2]]
      : gameBoard.diagonal[cell.classList[2].slice(9)];
  }
  function isLineComplete(filtered) {
    return (
      filtered.length == 3 && sameMark(filtered[0], filtered[1], filtered[2])
    );
  }

  function sameMark(cell1, cell2, cell3) {
    return (
      getMark(cell1) == getMark(cell2) &&
      getMark(cell2) == getMark(cell3) &&
      getMark(cell1) == getMark(cell3)
    );
  }
  ...
  function switchMarks(e) {
    ...
    let affectedMark =
      targetMark.textContent == player1Marker.textContent
        ? player2Marker
        : player1Marker;
